...
os.chdir(root)
if make_backup:     # I have no idea why this is causing an error. But it's working lol
    ...
    for directory in os.listdir(dir_backup_folder):     # for each directory in the backup
        if os.path.isdir(os.path.join(dir_backup_folder, directory)):
            os.chdir(os.path.join(dir_backup_folder, directory))
            for file in os.listdir(os.path.join(dir_backup_folder, directory)):  # copying all files to root
